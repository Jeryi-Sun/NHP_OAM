...
class MyDataset(Dataset):
    def __init__(self, main_file_path, all_file, session_len=10):
        self.df = pd.read_csv(main_file_path)

        self.all_df = pd.read_csv(all_file)
    ...
    def get_intervel_features(self, now_time, time):
        inter_time_min = (now_time-time)/1000/60 + 1 # 0是 padding
        return inter_time_min
    
    def __getitem__(self, idx):
        user_id, time, label, rec_pos, src_pos, query_pos = self.df.loc[idx, ['user_id', 'request_time_ms', 'label', 'rec_pos','src_pos', 'query_pos']]
        rec_pos, src_pos, query_pos = int(rec_pos), int(src_pos), int(query_pos)
        if rec_pos>0:
            rec_pos -= 1
        ...
        const.ours_session_len = self.session_len
        ...
        src_poses = filtered_df['src_pos'].values.tolist()  # 获取所有 rec_pos
        ...
        search_inter_history = self.search_inter_history[user_id][0][:src_pos]  # 获取用户交互记录
        search_inter_history = [item_vocab[i] for i in search_inter_history]
        search_inter_time =  self.search_inter_history[user_id][1][:src_pos]
        search_sessions = []  # 初始化 session 列表
        search_sessions_time = []  # 初始化 session 列表
        ...
        # 再对 search 进行操作
        for i in range(len(src_poses)):  # 倒序遍历 src_pos
            # 每两个 src_pos 之间的 item 构成一个 session
            if i == 0:
                session = search_inter_history[:src_poses[0]]
            else:
                start = src_poses[i-1]
                end = src_poses[i]
                session = search_inter_history[start:end]

            # 若 session 长度不足 10，前填充 0
            search_sessions.append(session)

            if i == 0:
                session_time = search_inter_time[:query_poses[0]]
            else:
                start = src_poses[i-1]
                end = src_poses[i]
                session_time = search_inter_time[start:end] 

            search_sessions_time.append(session_time)
        ...
        search_sessions = pad_or_truncate_2d(search_sessions, const.ours_session_len, const.ours_max_search_len)
        ...
        search_inter_time = pad_or_truncate_2d(search_sessions_time, const.ours_session_len, const.max_seq_len_search)
        ...
        # 找到最小非零值
        min_val = find_min_value(query_inter_time, search_inter_time, rec_inter_time, session_time_list)

        # 对四个列表进行归一化处理
        query_sessions_time_normalized = normalization_time(query_inter_time, min_val)
        search_sessions_time_normalized = normalization_time(search_inter_time, min_val)
        ...
        #time = (time-min_val)/1000/60+1
        lag_time = min(int((time-min_val)/1000/60-max(rec_sessions_time_normalized[-1][-1], query_sessions_time_normalized[-1][-1])+1), const.ours_lag_time_vocab)


        return rec_sessions, search_sessions, query_sessions,  \
...
def normalization_time(time_list, min_value):
    if isinstance(time_list[0], list):  # 如果是2D列表
        normalized_list = [[(value-min_value)/1000/60 +1 if value > 0 else 0 for value in sub_list] for sub_list in time_list]
    else:  # 如果是1D列表
        normalized_list = [(value-min_value)/1000/60 +1 if value > 0 else 0 for value in time_list]
...
def my_collate_fn(batch):
    ...
    search_sessions_time_normalized = torch.tensor(search_sessions_time_normalized).float()
