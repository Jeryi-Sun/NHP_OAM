...
from models.transformer.Models import get_non_pad_mask
...
def compute_integral_unbiased(model, data, time, non_pad_mask, type_mask):
    ...
    temp_hid = model.THP.linear(data)[:, 1:, :]
    temp_hid = torch.sum(temp_hid * type_mask[:, 1:, :], dim=2, keepdim=True)

    all_lambda = softplus(temp_hid + model.THP.alpha * temp_time, model.THP.beta)
...
def log_likelihood(model, data, time, types, query_ratio=None):
    """ Log-likelihood of sequence. """

    non_pad_mask = get_non_pad_mask(types).squeeze(2)

    type_mask = torch.zeros([*types.size(), model.THP.num_types], device=data.device)
    for i in range(model.THP.num_types):
        type_mask[:, :, i] = (types == i + 1).bool().to(data.device)

    all_hid = model.THP.linear(data)
    if query_ratio is not None:
        all_lambda = softplus(all_hid + torch.cat([1-query_ratio.unsqueeze(2), query_ratio.unsqueeze(2)], dim=-1), model.THP.beta)
    else:
        all_lambda = softplus(all_hid, model.THP.beta)
...
def type_loss(prediction, types, loss_func):
    ...
    truth = types[:, 1:] - 1
    ignore_mask = (truth != -1)
    truth = truth.clamp(min=0)
    prediction = torch.sigmoid(prediction.squeeze())

    # 计算损失
    loss = loss_func(prediction, truth.to(torch.float))

    # 应用掩码
    loss = loss[ignore_mask]

    # 计算最终损失
    final_loss = loss.sum() / ignore_mask.sum()


    return final_loss


def type_loss_last(prediction, types, loss_func):
    """ Event prediction loss, cross entropy or label smoothing. """
    ...
    # 计算损失
    loss = loss_func(prediction, truth.to(torch.float)).mean()
