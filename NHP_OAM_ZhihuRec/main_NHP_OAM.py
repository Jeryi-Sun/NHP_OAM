...
parser.add_argument('--log', type=str, help='exp details, used for log name', default='NHPOAM')
...
parser.add_argument('--early_stop', type=int, help='early_stop_num', default=5)
...
# Set up logging
logging.basicConfig(filename=f'./logs/{args.log}_log_{dt_string}.log', level=logging.INFO, 
...
def train(model, train_loader, val_loader, test_loader, lr=0.001, epochs=10, device='cpu', save_path=None):
    ...
    for epoch in tqdm(range(epochs)):
        model.train()
        for rec_sessions, query_sessions,  \
            ...
            loss = model.train_(rec_sessions, query_sessions,  
                                query_sessions_time_normalized, rec_sessions_time_normalized, session_time_list_normalized, 
        ...
        with torch.no_grad():
            for rec_sessions, query_sessions,  \
                ...
                output = model.infer_(rec_sessions, query_sessions,  
                                      query_sessions_time_normalized, rec_sessions_time_normalized, session_time_list_normalized, 
...
if __name__ == '__main__':
    # 加载数据
    train_dataset = MyDataset(const.train_file, const.all_train_file, const.valid_file, const.test_file)
    val_dataset = MyDataset(const.valid_file,const.all_train_file, const.valid_file, const.test_file)
    test_dataset = MyDataset(const.test_file, const.all_train_file, const.valid_file, const.test_file)

    train_loader = DataLoader(train_dataset, batch_size=128, shuffle=False, collate_fn=my_collate_fn, pin_memory = True, prefetch_factor = args.batch_size // 16 * 2, num_workers = 16)
    val_loader = DataLoader(val_dataset, batch_size=512, collate_fn=my_collate_fn, pin_memory = True, prefetch_factor = args.batch_size // 16 * 2, num_workers = 16)
    test_loader = DataLoader(test_dataset, batch_size=512, collate_fn=my_collate_fn, pin_memory = True, prefetch_factor = args.batch_size // 16 * 2, num_workers = 16)
