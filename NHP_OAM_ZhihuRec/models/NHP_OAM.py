...
class NHP_OAM(nn.Module):
    def __init__(self, Model_param:ModelArgs):
        ...
        self.transformer_layer = nn.TransformerEncoderLayer(d_model=item_dim, nhead=1, batch_first=True)
    ...
    def forward(self, rec_sessions, query_sessions,  
        ...
        his_rep = torch.cat([rec, query], dim=2)
    ...
    def train_(self, rec_sessions, query_sessions,  
            query_sessions_time_normalized, rec_sessions_time_normalized, session_time_list_normalized, 
            user_id, time, lag_time, label, session_label_list):
        
        enc_out, prediction, pred, query_ratio = self.forward(rec_sessions,  query_sessions,  
            query_sessions_time_normalized,  rec_sessions_time_normalized, session_time_list_normalized, 
            user_id, time, lag_time, session_label_list)
        ...
        scale_time_loss = 100
        loss = 0.001 * event_loss + pred_loss #+ se / scale_time_loss
        #loss = pred_loss
        
        return loss

    @torch.inference_mode()
    def infer_(self, rec_sessions,  query_sessions,  
            query_sessions_time_normalized,  rec_sessions_time_normalized, session_time_list_normalized, 
            user_id, time, lag_time, session_label_list):
        
        _, _, pred, query_ratio = self.forward(rec_sessions,  query_sessions,  
            query_sessions_time_normalized, rec_sessions_time_normalized, session_time_list_normalized, 
            user_id, time, lag_time, session_label_list)
